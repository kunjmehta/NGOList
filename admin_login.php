...
<html>
<head>
	<title>NGOList-Admin Login</title>
	<meta charset="utf-8">
			...
			<form method="post" action="admin_login.php">
			    ...
			    <div class="button-container">
				    <div class="form-group col-md-12 col-xs-12 col-sm-12 text-center">
				    	<button type="submit" class="btn btn-lg col-md-12 col-xs-12 col-sm-12" name="login_button_admin">Login</button>
				    </div>
				</div>
		  	</form>
...
<?php
	...
	if (isset($_POST['login_button_admin'])) {
		$username = mysqli_real_escape_string($conn, $_POST["username"]);
		$password = mysqli_real_escape_string($conn, $_POST["password"]);

		$password_encrypt = md5($password);
		$query_user = "SELECT * FROM admin WHERE username='$username'";
		$query_pass = "SELECT * FROM admin WHERE password='$password_encrypt'";
